...
class Model_app extends CI_Model{
    ...
    public function getKodePromo(){
        $q = $this->db->query("select MAX(RIGHT(kd_promo,3)) as kd_max from tbl_promo");
        $kd = "";
        if($q->num_rows()>0){
            foreach($q->result() as $k){
                $tmp = ((int)$k->kd_max)+1;
    ...
    function getAllDataPenjualan(){
        return $this->db->query("SELECT 
                ...
                a.tanggal_penjualan,
                a.total_harga,
			    (select count(kd_penjualan) as sum from tbl_penjualan_detail where kd_penjualan=a.kd_penjualan) as jumlah
    ...
    function getLapPenjualan($tgl_awal,$tgl_akhir){
        return $this->db->query("SELECT * from tbl_penjualan_header
                left join tbl_pelanggan on tbl_penjualan_header.kd_pelanggan=tbl_pelanggan.kd_pelanggan
                where tanggal_penjualan between '$tgl_awal' and '$tgl_akhir'
                ")->result();
    }

    function getLapPenjualan2($tgl_awal,$tgl_akhir){
        return $this->db->query("SELECT kd_penjualan, sum(total_harga) as total_all from tbl_penjualan_header
                where tanggal_penjualan between '$tgl_awal' and '$tgl_akhir'
                ")->result();
    }
